...
function formatTime(minutes, seconds) {
  const minNew = String(minutes).padStart(2, '0');
  const secNew = String(seconds).padStart(2, '0');
  return `${minNew}:${secNew}`;
}
...
function isPalindrome(str) {
  const repl = /[\W_]/g;
  const lowCaseStr = str.toLowerCase().replace(repl, '');
...
function findLongestWord(sentence) {
  const sentSplited = sentence.split(' ');
  const arr = sentSplited.sort((a, b) => b.length - a.length);
  return arr[0];
}
...
function reverseWords(str) {
  return str
    .split(' ')
    .map((strItem) => strItem.split('').reverse().join(''))
    .join(' ');
}
...
function encodeToRot13(str) {
  return str.replace(
    ...
    (n) =>
      'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'[
        'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'.indexOf(n)
      ]
  );
}
